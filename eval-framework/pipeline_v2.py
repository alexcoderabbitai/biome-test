"""
GitHub Pull Request Data Collection Pipeline
=========================================

This script processes GitHub Pull Requests (PRs) in bulk, collecting comprehensive data
about each PR including comments, reviews, code changes, and commit details.
...
- pr_address: Full GitHub PR URL (e.g., https://github.com/owner/repo/pull/123)
- type: PR type identifier (16-17)
...
2. Run the script:
   ...
   ```
   python pipeline_v2.py --input custom_prs.csv
   ```

Output:
-------
- Creates 'eval_json' directory
- Generates JSON files for each PR type (pr_type_16.json, pr_type_17.json, etc.)
- Each JSON file contains detailed PR data including:
  - Basic PR metadata
  - Comments and reviews
  - Code changes and diffs
  - Commit details
  - File changes with hunks

Error Handling:
--------------
- Implements automatic retry for rate limits and network issues
- Logs errors and progress to console
- Saves data in batches to prevent data loss
"""
...
def run_pipeline(csv_file):
    ...
    type_groups = {str(i): [] for i in range(16, 18)}  # Initialize groups for types 1-4
    batch_size = 10
    processed_count = 0
...
if __name__ == '__main__':
    ...
    csv_file = 'pr_address_5.csv'  # Update this to the correct path if needed
